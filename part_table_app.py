...
def load_racks():
    ...
    for r, spaces in RACK_SPACES.items():
        ...
        grid = [
            [{"Part No": None, "Quantity": 0} for _ in range(cols)]
        ...
        with st.form("part_form"):
            ...
            if st.form_submit_button("Add/Update"):
                if pn:
                    ...
                    add_history(
                        "Master Update", "-", "-", "-", pn, 0, st.session_state.user
                    )
                    st.success(f"Updated master for {pn}")
        st.dataframe(
            pd.DataFrame.from_dict(
                st.session_state.part_master, orient="index"
            ).reset_index().rename(columns={"index": "Part No"})
        )
    ...
    with tab_objs[tabs.index("Input")]:
        ...
        with st.form("stock_form", clear_on_submit=True):
            ...
            part_no = st.selectbox(
                "Part No", options=sorted(st.session_state.part_master.keys())
            )
                ...
                if action == "Add":
                    if cell["Part No"] in (None, part_no):
                        if cell["Quantity"] + qty <= CELL_CAPACITY:
                            ...
                            cell["Quantity"] += qty
                            add_history(
                                "Add", rack_ui, row_ui, col_ui, part_no, qty, st.session_state.user
                            )
                            save_racks(st.session_state.racks)
                            st.success(
                                f"Added {qty} {part_no} in {rack_ui} R{row_ui}C{col_ui}"
                            )
                        else:
                    ...
                    if cell["Part No"] == part_no and cell["Quantity"] >= qty:
                        ...
                        if cell["Quantity"] == 0:
                            cell["Part No"] = None
                        add_history(
                            "Subtract", rack_ui, row_ui, col_ui, part_no, qty, st.session_state.user
                        )
                        save_racks(st.session_state.racks)
                        st.success(
                            f"Subtracted {qty} from {rack_ui} R{row_ui}C{col_ui}"
                        )
                    else:
...
if can_output:
    with tab_objs[tabs.index("Output")]:
        ...
        rack_sel = st.selectbox(
            "Select Rack", options=list(st.session_state.racks.keys())
        )
        rows = []
        ...
        if st.button("Find FIFO"):
            ...
            if fifo:
                st.success(
                    f"Pick from Rack {fifo['Rack']} R{fifo['Row']} C{fifo['Col']} (Qty {cell['Quantity']})"
                )
            else:
